...
class Import(View):
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            running_tasks = TaskResult.objects.filter(task_name__in=['WZO_App.tasks.import_zip_codes', 'WZO_App.tasks.import_eort', 'WZO_App.tasks.import_workshops', 'WZO_App.tasks.import_veh', 'WZO_App.tasks.import_rules'], status__in=['PENDING', 'STARTED', 'RECEIVED', 'RETRY', 'PROGRESS']).values('task_id', 'task_name', 'status')
